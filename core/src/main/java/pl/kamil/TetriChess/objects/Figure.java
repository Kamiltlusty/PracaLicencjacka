...
public abstract class Figure {
    public abstract boolean isMoveLegal(Vector2 initialPosition, Vector2 finalPosition, Figure selectedFigure, Board board);
    ...
    protected boolean isNotBlocked(Vector2 initialPosition, Board board) {
        String fieldSignature = board.findFieldSignatureByCoordinates((int) initialPosition.x, (int) initialPosition.y);
    ...
    protected Tuple2<Vector2, Boolean> isPathFigureFree(Vector2 initialPosition, Vector2 finalPosition, Figure selectedFigure, Board board) {
        ...
        if (finalPosition.y != initialPosition.y && finalPosition.x != initialPosition.x) {
            ...
            while (!isFinalAgain) {
                ...
                // seek for figure
                String foundSignature = board.findFieldSignatureByCoordinates((int) checkPosX, (int) checkPosY);
            ...
            while (!isFinalAgain) {
                ...
                // seek for figure
                String foundSignature = board.findFieldSignatureByCoordinates((int) initialPosition.x, (int) checkPosY);
                if (!Objects.equals(foundSignature, "-1")) {
                    Vector2 fieldCoordinates = board.findFieldCoordinates(foundSignature);
                    Optional<Figure> foundFigure = board.findFigureByCoordinatesAndReturn(fieldCoordinates.x, fieldCoordinates.y);
                    if (foundFigure.isPresent()) {
            ...
            while (!isFinalAgain) {
                ...
                // seek for figure
                String foundSignature = board.findFieldSignatureByCoordinates((int) checkPosX, (int) initialPosition.y);
                if (!Objects.equals(foundSignature, "-1")) {
                    Vector2 fieldCoordinates = board.findFieldCoordinates(foundSignature);
                    Optional<Figure> foundFigure = board.findFigureByCoordinatesAndReturn(fieldCoordinates.x, fieldCoordinates.y);
                    if (foundFigure.isPresent()) {
